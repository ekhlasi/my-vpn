...
import { scanSocks4, scanSni, scanCandidates, expandCidr, SNI_PORTS, MAX_CANDIDATE_JOBS } from '../services/ip-scanner'
...
export async function handleAdminApi(request: Request, env: Env, url: URL): Promise<Response> {
  ...
  if (path === '/clean-ips/scan-candidates' && method === 'POST') {
    ...
    // (up to /24, ~256 addresses) is expanded into individual candidates
    // that all share that line's port/country.
    const candidates: CandidateInput[] = []
    const MAX_RAW_CANDIDATES = 5000 // safety cap on expanded-candidate count, independent of the ip×port job cap below
    ...
    const sni = url.hostname
    const { results, truncated } = await scanCandidates(candidates, sni, { ports, defaultCountry })
    if (!results.length) {
