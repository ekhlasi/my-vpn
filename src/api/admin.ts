...
import {
  ...
  resetVpnConfigSettings,
  listCustomCandidateIps,
  addCustomCandidateIps,
  deleteCustomCandidateIp,
  clearCustomCandidateIps,
} from '../db/queries'
...
export async function handleAdminApi(request: Request, env: Env, url: URL): Promise<Response> {
  ...
  if (path === '/config-slots' && method === 'GET') {
    const slots = await getConfigSlotSettings(env)
    return json({ slots })
  }

  if (path === '/config-slots' && method === 'POST') {
    const { slot, enabled } = await readJson(request)
    const validSlots: Record<string, string> = {
      auto1: 'slot_auto_1_enabled',
      auto2: 'slot_auto_2_enabled',
      approved1: 'slot_approved_1_enabled',
      approved2: 'slot_approved_2_enabled',
      approved3: 'slot_approved_3_enabled',
      trial: 'slot_trial_enabled',
    }
    const settingKey = validSlots[String(slot)]
    if (!settingKey) return json({ error: 'اسلات نامعتبر است' }, 400)
    await setSettings(env, { [settingKey]: enabled ? '1' : '0' })
    const slots = await getConfigSlotSettings(env)
    return json({ ok: true, slots })
  }

  if (path === '/config-slots/reset' && method === 'POST') {
    await resetVpnConfigSettings(env)
    const slots = await getConfigSlotSettings(env)
    const settings = await getAllSettings(env)
    delete (settings as any).admin_password_hash
    return json({ ok: true, slots, settings })
  }

  // ---- v10: admin-supplied candidate IPs for the clean-IP scan ----
  if (path === '/candidate-ips' && method === 'GET') {
    const ips = await listCustomCandidateIps(env)
    return json({ ips })
  }

  if (path === '/candidate-ips' && method === 'POST') {
    const { text } = await readJson(request)
    if (!text || !String(text).trim()) return json({ error: 'حداقل یک آی‌پی یا رنج وارد کنید' }, 400)
    const { added, skipped } = await addCustomCandidateIps(env, String(text))
    const ips = await listCustomCandidateIps(env)
    return json({ ok: true, added, skipped, ips })
  }

  const candidateIpMatch = path.match(/^\/candidate-ips\/(\d+)$/)
  if (candidateIpMatch && method === 'DELETE') {
    await deleteCustomCandidateIp(env, Number(candidateIpMatch[1]))
    const ips = await listCustomCandidateIps(env)
    return json({ ok: true, ips })
  }

  if (path === '/candidate-ips/clear' && method === 'POST') {
    await clearCustomCandidateIps(env)
    return json({ ok: true, ips: [] })
  }
