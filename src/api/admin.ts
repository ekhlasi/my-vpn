...
import { scanSocks4, scanSni, discoverAndValidate } from '../services/ip-scanner'
...
export async function handleAdminApi(request: Request, env: Env, url: URL, ctx: ExecutionContext): Promise<Response> {
  ...
  // ---- Discovery (stage 1) + validation (stage 2), the two-step clean-IP mechanism ----
  // Stage 1: tunnel a DoH query through one of DISCOVERY_PROXIES (a chosen
  // country, or all) to find the Cloudflare edge IP for that network path —
  // candidates only. Stage 2: validate every unique candidate directly
  // (sniHandshake, no proxy) — this never runs in the opposite order.
  //
  // Streamed as newline-delimited JSON so the admin can watch every small
  // step (proxy connect, SOCKS4 reply, DoH query/response, each candidate's
  // validation) live instead of staring at a blank box for several seconds.
  if (path === '/discover-clean-ip' && method === 'POST') {
    const { country } = await readJson(request)
    const sni = url.hostname
    const encoder = new TextEncoder()
    const { readable, writable } = new TransformStream()
    const writer = writable.getWriter()
    const writeLine = (obj: unknown) => writer.write(encoder.encode(JSON.stringify(obj) + '\n')).catch(() => {})

    const work = (async () => {
      try {
        const { results, details } = await discoverAndValidate(String(country || 'ALL'), sni, (line) => {
          writeLine({ type: 'log', line })
        })
        await writeLine({ type: 'done', ok: true, sni, results, details })
      } catch (err) {
        await writeLine({ type: 'done', ok: false, error: err instanceof Error ? err.message : String(err) })
      } finally {
