...
import { IRAN_VALID_PORTS } from '../services/clean-ips'
...
export async function handleAdminApi(request: Request, env: Env, url: URL): Promise<Response> {
  ...
  // Country/port scan settings: which candidate pools (global + custom
  // country lists) and which Cloudflare ports to include in the rotation.
  if (path === '/clean-ips/settings' && method === 'GET') {
    const [pools, ports] = await Promise.all([getCleanIpPools(env), getCleanIpScanPorts(env)])
    return json({ pools, ports, validPorts: IRAN_VALID_PORTS })
  }

  if (path === '/clean-ips/settings' && method === 'POST') {
    const body = await readJson(request)
    if (Array.isArray(body.pools)) {
      await setCleanIpCustomPools(env, body.pools.filter((p: any) => p && !p.builtin))
    }
    if (Array.isArray(body.ports)) {
      await setCleanIpScanPorts(env, body.ports.map((n: any) => Number(n)).filter((n: number) => Number.isFinite(n)))
    }
    const [pools, ports] = await Promise.all([getCleanIpPools(env), getCleanIpScanPorts(env)])
    return json({ ok: true, pools, ports, validPorts: IRAN_VALID_PORTS })
  }
