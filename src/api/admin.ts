...
export async function handleAdminApi(request: Request, env: Env, url: URL): Promise<Response> {
  ...
  // Per-country breakdown for the "آی‌پی تمیز" tab's country grid.
  if (path === '/clean-ips/countries' && method === 'GET') {
    const countries = await listCleanIpsByCountry(env)
