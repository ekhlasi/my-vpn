...
export async function handleAdminApi(request: Request, env: Env, url: URL): Promise<Response> {
  ...
  // `logs` collects every granular step from both stages (in order) so the
  // dashboard can show exactly what happened, even though it's delivered as
  // one normal JSON response like every other endpoint here.
  if (path === '/discover-clean-ip' && method === 'POST') {
    ...
    return json({ ok: true, sni, results, details, logs })
  }
