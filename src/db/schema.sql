...
-- Atomic daily request counter. Keeping the hot counter out of the shared
-- settings key/value row prevents concurrent dashboard/service requests from
-- racing on read-modify-write operations.
...
-- Per-slot enable/disable for generated configs (requirement: "گزینه فعال یا
-- غیرفعال هر کدوم" — like a real VPN provider's admin panel, each of the 5
-- pro server slots — and the trial slot — can be turned off independently.
-- A disabled pro slot is simply omitted from that user's subscription list;
-- a disabled trial slot falls back to a plain (non-clean-IP) config instead
-- of disappearing, since a trial user only ever gets one config and handing
-- them zero would look like a broken/expired account rather than a
-- deliberate admin choice. See buildUserSubscription in services/subscription.ts.
...
-- Admin-supplied extra candidate IPs/CIDRs to test for the clean-IP scan
-- (requirement: get real German/Turkish-routed IPs, same as other
-- providers give their users). CANDIDATE_IPS in services/clean-ips.ts is a
-- small fixed, manually-curated list; real-world "healthy IP" lists shared
-- in Iranian VPN/Telegram communities are updated far more often than any
-- hardcoded array in this repo can be, so this table lets the admin paste
-- in a trusted, frequently-refreshed list and have THIS system independently
-- re-verify it against Iranian vantage points before anything is approved
-- for users — see docs/troubleshooting.md and CHANGELOG-v10.md.
CREATE TABLE IF NOT EXISTS custom_candidate_ips (
  id         INTEGER PRIMARY KEY AUTOINCREMENT,
  value      TEXT NOT NULL UNIQUE,  -- single IPv4 address or CIDR block (network address is tested)
  label      TEXT,                  -- optional admin note, e.g. "کانال X - claimed آلمان"
  added_at   INTEGER NOT NULL
);
