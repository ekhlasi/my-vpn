...
-- `colo`/`country` (added in migration-v9, requirement: group clean IPs by
-- country like the Z-E-U-S scanner's per-country lists) come straight from
-- the `colo=` field in the /cdn-cgi/trace response returned by that specific
-- IP — see src/services/cf-colos.ts for the colo->country lookup.
