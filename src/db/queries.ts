...
export interface CleanIpCountrySummary {
  country: string
...
/**
 * Healthy-clean-IP counts grouped by country (see services/cf-colos.ts),
 * best latency first within the ranking — powers the country grid in the
 * admin panel's "آی‌پی تمیز" tab, mirroring the Z-E-U-S scanner's per-country
 * clean-IP breakdown. Rows with no resolvable country (colo unknown, or the
 * IP was tested before migration-v9 added colo/country) are excluded here;
 * they still show up in the flat listCleanIps() table.
 */
export async function listCleanIpsByCountry(env: Env): Promise<CleanIpCountrySummary[]> {
  const { results } = await env.DB.prepare(
    `SELECT
       country,
       SUM(healthy) AS healthy_count,
       COUNT(*) AS total_count,
       MIN(CASE WHEN healthy = 1 THEN latency_ms END) AS best_latency_ms
     FROM clean_ips
     WHERE country IS NOT NULL
     GROUP BY country
     HAVING healthy_count > 0
     ORDER BY healthy_count DESC, best_latency_ms ASC`,
  ).all<CleanIpCountrySummary>()
  return results ?? []
}
