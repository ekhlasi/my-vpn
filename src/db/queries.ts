...
import { discoverCleanIpsBatch, CANDIDATE_IPS, parseCandidateInput, type CleanIpResult } from '../services/clean-ips'
...
const SETTINGS_DEFAULTS: Record<string, string> = {
  ...
  wiki_gift_link: '',
  // ---- Per-slot config enable/disable, like a real VPN provider's admin
  // panel lets you turn individual servers on/off (v10). See
  // buildUserSubscription in services/subscription.ts. ----
  slot_auto_1_enabled: '1',
  slot_auto_2_enabled: '1',
  slot_approved_1_enabled: '1',
  slot_approved_2_enabled: '1',
  slot_approved_3_enabled: '1',
  slot_trial_enabled: '1',
}

/**
 * Settings keys that make up "VPN config behavior" as a group — the ones a
 * "بازگشت به تنظیمات پیش‌فرض" (reset to defaults) button restores (v10).
 * Deliberately narrow: things like the Telegram bot token, admin password,
 * or user data are never touched by this. Resetting means deleting the
 * override row so getSetting/getAllSettings fall back to SETTINGS_DEFAULTS
 * above, not writing some separately-tracked "default" value that could
 * itself drift out of sync with SETTINGS_DEFAULTS over time.
 */
export const VPN_CONFIG_RESETTABLE_KEYS = [
  'pro_config_name',
  'trial_config_name',
  ...
  'slot_auto_1_enabled',
  'slot_auto_2_enabled',
  'slot_approved_1_enabled',
  'slot_approved_2_enabled',
  'slot_approved_3_enabled',
  'slot_trial_enabled',
] as const

/** Restores every key in VPN_CONFIG_RESETTABLE_KEYS to its SETTINGS_DEFAULTS value (requirement: "بازگشت به تنظیمات پیش‌فرض"). */
export async function resetVpnConfigSettings(env: Env): Promise<void> {
  const stmts = VPN_CONFIG_RESETTABLE_KEYS.map((k) => env.DB.prepare('DELETE FROM settings WHERE key = ?').bind(k))
  await env.DB.batch(stmts)
}

/** The 6 config-slot toggle values, each coerced to a plain boolean (defaults to on if unset). */
export interface ConfigSlotSettings {
  auto1: boolean
  auto2: boolean
  approved1: boolean
  approved2: boolean
...
export async function getConfigSlotSettings(env: Env): Promise<ConfigSlotSettings> {
  const s = await getAllSettings(env)
  return {
    auto1: s.slot_auto_1_enabled !== '0',
    auto2: s.slot_auto_2_enabled !== '0',
    approved1: s.slot_approved_1_enabled !== '0',
    approved2: s.slot_approved_2_enabled !== '0',
    approved3: s.slot_approved_3_enabled !== '0',
    trial: s.slot_trial_enabled !== '0',
  }
}
...
export async function getQuotaStatus(env: Env): Promise<QuotaStatus> {
  ...
  let quotaRow: { count: number } | null = null
  try {
    quotaRow = await env.DB.prepare('SELECT count FROM request_quota WHERE day = ?').bind(today).first<{ count: number }>()
  } catch (err) {
    // Backward compatibility: a database that has not received migration-v11
    // yet can still render the dashboard using the legacy settings counter.
    console.warn('request_quota table unavailable; using legacy quota counter:', err)
  }
  const sameDay = settings.usage_quota_date === today
  // v11 stores the hot counter in its own table. Fall back to the legacy
  // settings value so an older database remains readable until migration runs.
  const count = quotaRow?.count != null
    ? Number(quotaRow.count) || 0
    : (sameDay ? parseFloat(settings.usage_quota_count) || 0 : 0)
...
export async function checkAndIncrementQuota(env: Env): Promise<{ allowed: boolean; paused: boolean }> {
  ...
  // v11: atomic UPSERT. This avoids the classic lost-update race where two
  // simultaneous requests both read N and both write N+1 to the shared
  // settings row. It also keeps quota accounting off the settings hot row.
  let nextCount: number
  try {
    await env.DB.prepare(
      `INSERT INTO request_quota (day, count) VALUES (?, 1)
       ON CONFLICT(day) DO UPDATE SET count = request_quota.count + 1`,
    ).bind(today).run()
    const row = await env.DB.prepare('SELECT count FROM request_quota WHERE day = ?').bind(today).first<{ count: number }>()
    nextCount = Number(row?.count) || 1
  } catch (err) {
    // Migration-v11 may not have landed yet. Keep old deployments functional
    // instead of turning quota accounting into a service-wide failure.
    console.warn('request_quota table unavailable; using legacy counter:', err)
    const legacySameDay = settings.usage_quota_date === today
    nextCount = (legacySameDay ? parseFloat(settings.usage_quota_count) || 0 : 0) + 1
    await setSettings(env, { usage_quota_date: today, usage_quota_count: String(nextCount) })
  }
  ...
  if (!overLimit && currentlyPaused && pausedReason === 'auto') {
    // New quota day, or an admin-raised limit: lift an automatic pause before
    // serving the request. Manual pauses are never cleared here.
    await setSettings(env, {
      service_paused: '0',
      service_paused_reason: '',
      usage_quota_date: today,
    })
    return { allowed: true, paused: false }
  }

  if (overLimit && autoPause) {
    // Only write the pause flag when it isn't already the same automatic
    // pause. Requests above the limit therefore perform no repeated settings
    // writes.
    if (!(currentlyPaused && pausedReason === 'auto')) {
      await setSettings(env, {
        service_paused: '1',
        service_paused_reason: 'auto',
        usage_quota_date: today,
      })
    }
    return { allowed: false, paused: true }
  ...
  // Keep the legacy day marker in sync without writing the hot counter on
  // every request. This also makes a later rollback to pre-v11 code safer.
  if (settings.usage_quota_date !== today) {
...
async function saveCleanIpResults(env: Env, results: CleanIpResult[]): Promise<void> {
  ...
  const statements = results.map((r) =>
    ...
    ).bind(r.ip, r.latencyMs, r.healthy ? 1 : 0, checkedAt, r.error, r.colo, r.country),
  )
  // A single D1 batch is materially cheaper/safer than N independent writes
  // and prevents a clean-IP scan from occupying the D1 write queue longer
  // than necessary.
...
export async function setCleanIpsApproved(env: Env, ips: string[], approved: boolean): Promise<void> {
  ...
  const statements = ips.map((ip) =>
    env.DB.prepare(`UPDATE clean_ips SET approved = ? WHERE ip = ?`).bind(approved ? 1 : 0, ip),
  )
  await env.DB.batch(statements)
}
...
export async function runCleanIpDiscovery(env: Env, domain: string, batchSize?: number): Promise<CleanIpResult[]> {
  // v10: merge in the admin's own pasted candidate list (custom_candidate_ips)
  // alongside the built-in CANDIDATE_IPS, so a fresher, community-sourced
  // "German/Turkish clean IP" list can be independently re-verified from
  // Iran instead of only ever cycling the small hardcoded array — see
  // CHANGELOG-v10.md.
  const custom = await listCustomCandidateIps(env)
  const merged = custom.length ? [...CANDIDATE_IPS, ...custom.map((c) => c.value)] : CANDIDATE_IPS
  const offsetStr = await getSetting(env, 'clean_ip_scan_offset')
  const offset = parseInt(offsetStr, 10) || 0
  const results = await discoverCleanIpsBatch(domain, offset, batchSize, merged)
  await saveCleanIpResults(env, results)
  await setSetting(env, 'clean_ip_scan_offset', String((offset + results.length) % merged.length))
  return results
}

// ==================== Admin-supplied candidate IPs (v10) ====================

export interface CustomCandidateIpRow {
  id: number
  value: string
  label: string | null
  added_at: number
...
export async function listCustomCandidateIps(env: Env): Promise<CustomCandidateIpRow[]> {
  const { results } = await env.DB.prepare('SELECT * FROM custom_candidate_ips ORDER BY added_at DESC').all<CustomCandidateIpRow>()
  return results ?? []
}

/**
 * Parses a multi-line paste (one IP or CIDR per line, optional trailing
 * `# label` / `- label` comment) and inserts the valid, de-duplicated
 * entries. Returns how many were actually added (already-present values are
 * silently skipped via INSERT OR IGNORE, not counted as an error).
 */
export async function addCustomCandidateIps(env: Env, rawText: string): Promise<{ added: number; skipped: string[] }> {
  const { entries, skipped } = parseCandidateInput(rawText)
  let added = 0
  for (const e of entries) {
    const res = await env.DB.prepare(
      'INSERT OR IGNORE INTO custom_candidate_ips (value, label, added_at) VALUES (?, ?, ?)',
    )
      .bind(e.value, e.label ?? null, Date.now())
      .run()
    if (res.meta?.changes) added += res.meta.changes
  }
  return { added, skipped }
}

export async function deleteCustomCandidateIp(env: Env, id: number): Promise<void> {
  await env.DB.prepare('DELETE FROM custom_candidate_ips WHERE id = ?').bind(id).run()
}

export async function clearCustomCandidateIps(env: Env): Promise<void> {
  await env.DB.prepare('DELETE FROM custom_candidate_ips').run()
}
