...
export async function handleRequest(
  ...
  try {
    ...
    // Admin traffic is control-plane traffic, not VPN service traffic. In
    // particular, opening the dashboard must never start a clean-IP scan or
    // enqueue a quota-counter write. The old implementation did both for
    // every request, so the dashboard's ~10 parallel API calls could compete
    // with the clean-IP worker and the quota writer for the same D1 database.
    // That contention is especially visible after the clean-IP feature is
    // enabled: cards stay on "در حال بارگذاری…" even though the page itself
    // rendered correctly.
    const isAdminTraffic =
      earlyUrl.pathname === '/' ||
      earlyUrl.pathname === '/admin' ||
      earlyUrl.pathname === '/admin/' ||
      earlyUrl.pathname === '/admin/login' ||
      earlyUrl.pathname.startsWith('/api/admin/')
    ...
    if (env.DB && !isAdminTraffic) {
      try {
        quotaPaused = (await getQuotaStatus(env)).paused
      } catch (err) {
        console.error('getQuotaStatus failed (treating as not paused):', err)
      }
      // Do not make quota accounting part of the response path. It is a
      // best-effort service metric and must never be able to take the admin
      // control plane down.
      ctx.waitUntil(
