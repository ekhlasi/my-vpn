  ...
  try {
    ...
    if (uuidLike && env.DB) {
      ...
      if (user) {
        ...
        // so on its own it can only prove Cloudflare-to-Cloudflare
        // reachability, not "a real Iranian ISP can reach this" — see the
        // limitation notice at the top of that file. Because of that this
        // stays strictly opt-in: only fetched (and only ever used) when the
        // admin has explicitly turned `auto_clean_ip_enabled` on in the
        // panel. Left off (the default), every config's connect address
        // (`add=`) is simply the worker's own host, same as before this
        // feature existed.
        const cleanIpChoice = settings.auto_clean_ip_enabled === '1' ? await getBestCleanIpForConfig(env) : null
