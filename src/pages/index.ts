...
export const baseStyles = `
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .bg-watermark::before {
    ...
    animation: bgWatermarkSpin 100s linear infinite;
    user-select: none;
  }
  [data-theme="dark"] .bg-watermark::before { color: color-mix(in srgb, var(--primary-light) 9%, transparent); }
  @keyframes bgWatermarkSpin { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }
  @media (prefers-reduced-motion: reduce) { .bg-watermark::before { animation: none; } }
...
export async function subscriptionPage(env: any, request: Request): Promise<Response> {
  ...
  const cardsHtml = subscriptions.map((sub: any) => `
    <div class="glass card" data-uuid="${sub.uuid}">
      <div style="display:flex;justify-content:space-between;align-items:center;gap:0.6rem;margin-bottom:1rem;flex-wrap:wrap;">
        <h3 style="color:var(--primary);font-size:1.05rem;">🦊 VLESS</h3>
        <span class="uuid-badge" style="font-size:0.72rem;color:#fff;padding:0.2rem 0.75rem;border-radius:30px;font-weight:600;">${sub.uuid.substring(0,8)}</span>
      </div>
