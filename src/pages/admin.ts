  ...
  <div class="dash-shell">
    ...
    <div class="tab-panel" id="tab-pool">
      ...
      <div class="glass" style="margin-bottom:1.2rem;">
        ...
        <p style="font-size:0.75rem; opacity:0.6; margin-top:0.7rem;">
          ⚠️ برچسب «کشور/دیتاسنتر» یک راهنمای احتمالاتی است، نه تضمین قطعی — این برچسب از خودِ کلادفلر و از مسیر این
          ورکر پرسیده می‌شود، در حالی که اینترنت هر اپراتور ایرانی ممکن است طبق مسیر BGP خودش به یک دیتاسنتر متفاوت
          از همین آی‌پی anycast برسد. ستون «وضعیت» (که واقعاً از گره‌های داخل ایران تست می‌شود) قابل‌اعتمادتر از
          برچسب کشور است — برای اطمینان بیشتر، آی‌پی‌های تأییدشده را روی خط خودتان هم امتحان کنید.
        </p>
      </div>

      <div class="glass" style="margin-bottom:1.2rem;">
        <h3 style="margin-bottom:0.6rem;">➕ افزودن آی‌پی/رنج پیشنهادی برای تست (مثل لیست‌های آلمان/ترکیه سایر ارائه‌دهندگان)</h3>
        <p style="font-size:0.8rem; opacity:0.7; margin-bottom:0.8rem;">
          لیست کوچک و ثابت بالا هرگز نمی‌تواند مثل کانال‌ها/جامعه‌ی وی‌پی‌ ایران، لحظه‌به‌لحظه به‌روز بماند. اینجا
          می‌توانید همان آی‌پی/رنج‌هایی که جای دیگری «سالم» یا «آلمان/ترکیه» معرفی شده‌اند را بچسبانید تا همین سیستم
          دوباره و مستقل از گره‌های داخل ایران تستشان کند — هر خط یک آی‌پی یا CIDR (مثلاً <code>104.16.4.0/24</code>)،
          با یک برچسب اختیاری بعد از <code>#</code>. نتیجه در همان جدول بالا (پس از زدن «شروع تست پیوسته») ظاهر می‌شود.
        </p>
        <div class="field">
          <textarea id="candidateIpsInput" rows="4" style="width:100%; font-family:monospace; font-size:0.85rem;" placeholder="104.16.4.0/24 # آلمان - کانال X
188.114.99.10 # ترکیه"></textarea>
        </div>
        <div style="display:flex; flex-wrap:wrap; gap:0.5rem; margin-top:0.5rem;">
          <button class="btn" id="candidateIpsAddBtn" type="button">➕ افزودن به فهرست تست</button>
          <button class="btn" id="candidateIpsClearBtn" type="button" style="background:color-mix(in srgb, var(--error) 15%, transparent); color:var(--error);">🗑️ پاک‌کردن کل فهرست دستی</button>
        </div>
        <div class="table-wrap" style="margin-top:0.8rem;">
          <table id="candidateIpsTable">
            <thead><tr><th>آی‌پی/رنج</th><th>برچسب</th><th>افزوده‌شده</th><th></th></tr></thead>
            <tbody><tr><td colspan="4">هنوز چیزی اضافه نشده</td></tr></tbody>
          </table>
        </div>
      </div>

      <div class="glass" style="margin-bottom:1.2rem;">
        <h3 style="margin-bottom:0.6rem;">⚙️ فعال/غیرفعال‌کردن هر اسلات کانفیگ (مثل پنل شرکت‌های وی‌پی‌ان)</h3>
        <p style="font-size:0.8rem; opacity:0.7; margin-bottom:0.8rem;">
          دقیقاً مثل پنل ادمین سرویس‌های وی‌پی‌ان تجاری، هر کدام از ۵ اسلات کانفیگ کاربر VIP (به‌علاوه‌ی کانفیگ تست) را
          می‌توانید جدا از بقیه خاموش کنید — اسلات خاموش‌شده اصلاً در لیست اشتراک آن کاربر نمایش داده نمی‌شود.
        </p>
        <div id="configSlotsGrid" style="display:grid; grid-template-columns:repeat(auto-fill,minmax(220px,1fr)); gap:0.6rem;"></div>
        <div style="margin-top:0.8rem;">
          <button class="btn" id="configSlotsResetBtn" type="button" style="background:color-mix(in srgb, var(--error) 15%, transparent); color:var(--error);">🔄 بازگشت همه‌ی تنظیمات کانفیگ به پیش‌فرض</button>
          <span style="font-size:0.75rem; opacity:0.6; margin-inline-start:0.5rem;">نام‌گذاری کانفیگ‌ها، روشن/خاموش‌بودن آی‌پی سالم، تنظیمات پنل‌ها و همه‌ی اسلات‌های بالا را به حالت اولیه برمی‌گرداند (فهرست آی‌پی‌های دستی و کاربران دست‌نخورده می‌مانند).</span>
        </div>
      </div>
    </div>
  ...
  (function() {
    ...
    async function api(path, opts) {
      ...
      // Keep the dashboard from waiting forever when a D1/network operation
      // is genuinely stuck. A failed card becomes a visible toast instead of
      // remaining on "در حال بارگذاری…" indefinitely.
      options.signal = controller.signal;
      try {
        const res = await fetch(path, options);
        if (res.status === 401) { location.href = '/admin/login'; throw new Error('جلسه مدیریت منقضی شده است'); }
        const data = await res.json().catch(() => ({}));
        if (!res.ok) throw new Error(data.error || ('خطای HTTP ' + res.status));
        return data;
      } catch (err) {
        if (err && err.name === 'AbortError') throw new Error('پاسخ سرور بیش از ۱۵ ثانیه طول کشید؛ وضعیت D1/Deployment را بررسی کنید');
        throw err;
      } finally {
    ...
    // ---------- v10: admin-supplied candidate IPs ----------
    async function loadCandidateIps() {
      const data = await api('/api/admin/candidate-ips');
      const tbody = document.querySelector('#candidateIpsTable tbody');
      if (!data.ips.length) { tbody.innerHTML = '<tr><td colspan="4">هنوز چیزی اضافه نشده</td></tr>'; return; }
      tbody.innerHTML = data.ips.map(row =>
        '<tr>' +
          '<td style="font-family:monospace;">' + row.value + '</td>' +
          '<td style="font-size:0.8rem;">' + (row.label || '<span style=\"opacity:0.5;\">—</span>') + '</td>' +
          '<td style="font-size:0.72rem;">' + fmtDate(row.added_at) + '</td>' +
          '<td><button class="btn" data-del-candidate="' + row.id + '" type="button" style="padding:0.25rem 0.6rem; font-size:0.75rem; background:color-mix(in srgb, var(--error) 15%, transparent); color:var(--error);">حذف</button></td>' +
        '</tr>'
      ).join('');
      tbody.querySelectorAll('[data-del-candidate]').forEach((btn) => {
        btn.addEventListener('click', async () => {
          try { await api('/api/admin/candidate-ips/' + btn.getAttribute('data-del-candidate'), { method: 'DELETE' }); await loadCandidateIps(); }
          catch (e) { showToast(e.message, true); }
        });
      });
    }
    document.getElementById('candidateIpsAddBtn').addEventListener('click', async (e) => {
      const btn = e.target;
      const box = document.getElementById('candidateIpsInput');
      const text = box.value.trim();
      if (!text) { showToast('حداقل یک آی‌پی یا رنج وارد کنید', true); return; }
      btn.disabled = true;
      try {
        const res = await api('/api/admin/candidate-ips', { method: 'POST', body: JSON.stringify({ text }) });
        showToast(res.added + ' مورد اضافه شد' + (res.skipped && res.skipped.length ? (' — ' + res.skipped.length + ' خط نامعتبر رد شد') : ''));
        box.value = '';
        await loadCandidateIps();
      } catch (e) { showToast(e.message, true); }
      finally { btn.disabled = false; }
    });
    document.getElementById('candidateIpsClearBtn').addEventListener('click', async (e) => {
      if (!confirm('کل فهرست آی‌پی‌های دستی حذف شود؟ آی‌پی‌های تست‌شده و لیست ربات فعلی دست‌نخورده می‌مانند.')) return;
      const btn = e.target;
      btn.disabled = true;
      try { await api('/api/admin/candidate-ips/clear', { method: 'POST' }); showToast('فهرست دستی پاک شد'); await loadCandidateIps(); }
      catch (e) { showToast(e.message, true); }
      finally { btn.disabled = false; }
    });

    // ---------- v10: per-slot config enable/disable + reset to defaults ----------
    const CONFIG_SLOT_LABELS = [
      ['auto1', '🟢 سرور VIP ۱ (آی‌پی سالم خودکار)'],
      ['auto2', '🟢 سرور VIP ۲ (آی‌پی سالم خودکار)'],
      ['approved1', '✅ سرور VIP ۳ (آی‌پی تأییدشده)'],
      ['approved2', '✅ سرور VIP ۴ (آی‌پی تأییدشده)'],
      ['approved3', '✅ سرور VIP ۵ (آی‌پی تأییدشده)'],
      ['trial', '🎁 کانفیگ تست/ترایال'],
    ];
    function renderConfigSlotsGrid(slots) {
      const grid = document.getElementById('configSlotsGrid');
      grid.innerHTML = CONFIG_SLOT_LABELS.map(([key, label]) =>
        '<label class="glass" style="display:flex; align-items:center; justify-content:space-between; gap:0.5rem; padding:0.6rem 0.8rem; cursor:pointer;">' +
          '<span style="font-size:0.82rem;">' + label + '</span>' +
          '<input type="checkbox" data-slot="' + key + '" ' + (slots[key] ? 'checked' : '') + '>' +
        '</label>'
      ).join('');
      grid.querySelectorAll('[data-slot]').forEach((cb) => {
        cb.addEventListener('change', async () => {
          cb.disabled = true;
          try {
            await api('/api/admin/config-slots', { method: 'POST', body: JSON.stringify({ slot: cb.getAttribute('data-slot'), enabled: cb.checked }) });
            showToast('ذخیره شد');
    ...
    async function loadConfigSlots() {
      const data = await api('/api/admin/config-slots');
      renderConfigSlotsGrid(data.slots);
    }
    document.getElementById('configSlotsResetBtn').addEventListener('click', async (e) => {
      if (!confirm('همه‌ی تنظیمات کانفیگ (نام‌گذاری، آی‌پی سالم، پنل‌ها، اسلات‌ها) به پیش‌فرض کارخانه برگردد؟')) return;
      const btn = e.target;
      btn.disabled = true;
      try {
        const res = await api('/api/admin/config-slots/reset', { method: 'POST' });
        renderConfigSlotsGrid(res.slots);
        autoCleanIpOn = res.settings.auto_clean_ip_enabled === '1';
        renderAutoCleanIpToggle();
        showToast('تنظیمات کانفیگ‌ها به پیش‌فرض بازگشت');
      } catch (e) { showToast(e.message, true); }
      finally { btn.disabled = false; }
    });
    ...
    // Bug fixed here: these used to be fired with no .catch() at all. If any
    // one of them rejected (e.g. a backend error), it became a silent,
    // invisible unhandled promise rejection — that section of the dashboard
    // just stayed on "در حال بارگذاری…" forever with no toast and no way to
    // tell what went wrong. Every load call now reports failures visibly and
    // independently, so one broken section never looks like "the whole panel
    // is stuck" and always tells you (via toast) what actually failed.
    [
      ['loadOverview', loadOverview], ['loadUsers', loadUsers], ['loadBotUsers', loadBotUsers],
      ['loadTrialSettings', loadTrialSettings], ['loadQuota', loadQuota], ['loadPool', loadPool],
      ['loadSyncSecret', loadSyncSecret], ['loadCleanIps', loadCleanIps],
      ['loadAutoCleanIpToggle', loadAutoCleanIpToggle], ['loadDnsRules', loadDnsRules],
      ['loadCandidateIps', loadCandidateIps], ['loadConfigSlots', loadConfigSlots],
    ].forEach(([name, fn]) => {
      fn().catch((e) => { console.error(name, e); showToast((name + ': ' + (e && e.message || e)), true); });
    });
  })();
  </script>`
