...
export function dashboardPage(): Response {
  ...
  <div class="dash-shell">
    ...
    <div class="tab-panel" id="tab-pool">
      ...
      <div class="glass" style="margin-bottom:1.2rem;">
        ...
        <p style="font-size:0.8rem; opacity:0.7; margin-bottom:0.8rem;">
          این بخش چند آی‌پی از رنج‌های کلادفلر را مستقیماً از خود ورکر تست می‌کند. این تست فقط اتصال «کلادفلر به کلادفلر»
          را می‌سنجد — یعنی نمی‌تواند به‌تنهایی تضمین کند که آن آی‌پی از داخل ایران هم باز است، پس ممکن است یک آی‌پی را
          «سالم» نشان بدهد در حالی که برای برخی کاربران ایرانی فیلتر است. تست روی چند پورت مختلف و روی چند استخر آی‌پی
          (از جمله لیست‌هایی که خودتان برای کشورهای خاص وارد می‌کنید) این احتمال را کم می‌کند ولی به صفر نمی‌رساند. به همین
          دلیل جایگزینی خودکار آدرس اتصال (add) در کانفیگ‌ها پیش‌فرض <b>غیرفعال</b> است.
        </p>
        <div class="field" style="display:flex; align-items:center; gap:0.5rem; max-width:none;">
          ...
          <label for="autoCleanIp" style="margin:0;">جایگزینی خودکار آی‌پی تست‌شده به‌جای دامنه در کانفیگ‌ها</label>
        </div>
        <button class="btn" id="autoCleanIpSaveBtn" type="button" style="margin-top:0.6rem;">💾 ذخیره</button>

        <h4 style="margin:1.2rem 0 0.5rem;">پورت‌های اسکن (پورت‌های معتبر کلادفلر برای ایران)</h4>
        <p style="font-size:0.78rem; opacity:0.7; margin-bottom:0.5rem;">
          هر آی‌پی روی پورت‌های تیک‌خورده تست می‌شود. اگر یک رنج روی ۴۴۳ کند/مسدود بود، گاهی همان رنج روی یکی از پورت‌های
          دیگر باز می‌ماند.
        </p>
        <div id="cleanIpPortsBox" style="display:flex; flex-wrap:wrap; gap:0.8rem; margin-bottom:0.6rem;"></div>

        <h4 style="margin:1.2rem 0 0.5rem;">استخرهای آی‌پی برای اسکن (بر اساس کشور)</h4>
        <p style="font-size:0.78rem; opacity:0.7; margin-bottom:0.6rem;">
          «جهانی» همیشه فعال و داخلی است. برای هرکدام از کشورهای زیر می‌توانید لیست آی‌پی‌های مخصوص همان کشور را (هر
          خط یک آی‌پی) وارد و فعال کنید — این لیست‌ها همچنان از داخل ورکر تست می‌شوند، فقط دامنه‌ی کاندیدها را بیشتر
          می‌کنند.
        </p>
        <div id="cleanIpPoolsBox"></div>
        <button class="btn" id="cleanIpPoolsSaveBtn" type="button" style="margin-top:0.8rem;">💾 ذخیره استخرها و پورت‌ها</button>

        <div style="margin-top:1.2rem; display:flex; align-items:center; gap:0.6rem; flex-wrap:wrap;">
          <button class="btn" id="cleanIpsRefreshBtn" type="button">🔄 تست دسته بعدی همین الان</button>
          <button class="btn" id="cleanIpsContinuousBtn" type="button">▶️ شروع اسکن پیوسته</button>
          <span id="cleanIpsContinuousStatus" style="font-size:0.78rem; opacity:0.75;"></span>
        </div>

        <div class="table-wrap" style="margin-top:1rem;">
          <table id="cleanIpsTable">
            <thead><tr><th>آی‌پی</th><th>پورت</th><th>استخر</th><th>وضعیت</th><th>تأخیر (ms)</th><th>آخرین بررسی</th></tr></thead>
            <tbody><tr><td colspan="6">در حال بارگذاری…</td></tr></tbody>
          </table>
        </div>
      </div>
  ...
  (function() {
    ...
    const CLEAN_IP_PORT_LABELS = { 443: '443 (پیش‌فرض)', 2053: '2053', 2083: '2083', 2087: '2087', 2096: '2096', 8443: '8443' };
    let cleanIpScanTimer = null;
    ...
    async function loadCleanIpScanSettings() {
      const data = await api('/api/admin/clean-ips/settings');
      const selectedPorts = new Set(data.ports);
      const portsBox = document.getElementById('cleanIpPortsBox');
      portsBox.innerHTML = data.validPorts.map(p =>
        '<label style="display:flex; align-items:center; gap:0.35rem; font-size:0.85rem;">' +
          '<input type="checkbox" class="cleanIpPortCb" value="' + p + '" style="width:auto;" ' + (selectedPorts.has(p) ? 'checked' : '') + '>' +
          (CLEAN_IP_PORT_LABELS[p] || p) +
        '</label>'
      ).join('');

      const poolsBox = document.getElementById('cleanIpPoolsBox');
      poolsBox.innerHTML = data.pools.map(pool => {
        if (pool.builtin) {
          return '<div style="margin-bottom:0.7rem; font-size:0.82rem; opacity:0.75;">🌍 ' + pool.label + ' — همیشه فعال (داخلی، غیرقابل ویرایش)</div>';
        }
        const id = 'cleanIpPool_' + pool.key;
        return (
          '<div class="glass" style="padding:0.7rem; margin-bottom:0.7rem;">' +
            '<div style="display:flex; align-items:center; gap:0.5rem; margin-bottom:0.4rem;">' +
              '<input type="checkbox" class="cleanIpPoolEnabled" data-key="' + pool.key + '" style="width:auto;" ' + (pool.enabled ? 'checked' : '') + '>' +
              '<b>' + pool.label + '</b>' +
              '<span style="font-size:0.72rem; opacity:0.6;">(' + pool.ips.length + ' آی‌پی)</span>' +
            '</div>' +
            '<textarea id="' + id + '" class="cleanIpPoolIps" data-key="' + pool.key + '" data-label="' + pool.label + '" rows="4" ' +
              'placeholder="هر خط یک آی‌پی کلادفلر مخصوص ' + pool.label + '" ' +
              'style="width:100%; font-family:monospace; font-size:0.78rem;">' + pool.ips.join('\n') + '</textarea>' +
          '</div>'
        );
      }).join('');
    }

    document.getElementById('cleanIpPoolsSaveBtn').addEventListener('click', async (e) => {
      const btn = e.target;
      btn.disabled = true; const orig = btn.textContent; btn.textContent = 'در حال ذخیره…';
      try {
        const ports = Array.from(document.querySelectorAll('.cleanIpPortCb:checked')).map(cb => parseInt(cb.value, 10));
        const pools = Array.from(document.querySelectorAll('.cleanIpPoolIps')).map(ta => ({
          key: ta.dataset.key,
          label: ta.dataset.label,
          enabled: document.querySelector('.cleanIpPoolEnabled[data-key="' + ta.dataset.key + '"]').checked,
          ips: ta.value.split('\n').map(s => s.trim()).filter(Boolean),
        }));
        await api('/api/admin/clean-ips/settings', { method: 'POST', body: JSON.stringify({ ports, pools }) });
        showToast('استخرها و پورت‌ها ذخیره شد');
        loadCleanIpScanSettings();
      } catch (e) { showToast(e.message, true); }
      finally { btn.disabled = false; btn.textContent = orig; }
    });
    ...
    async function loadCleanIps() {
      ...
      if (!data.ips.length) { tbody.innerHTML = '<tr><td colspan="6">هنوز هیچ آی‌پی‌ای تست نشده — دکمه «تست دسته بعدی» یا «شروع اسکن پیوسته» را بزنید</td></tr>'; return; }
      tbody.innerHTML = data.ips.map(ip =>
        '<tr>' +
          '<td style="font-family:monospace;">' + ip.ip + '</td>' +
          '<td>' + ip.port + '</td>' +
          '<td>' + (ip.pool || 'global') + '</td>' +
    ...
    async function runOneCleanIpBatch() {
    ...
    document.getElementById('cleanIpsRefreshBtn').addEventListener('click', async (e) => {
      ...
      try {
        await runOneCleanIpBatch();
        showToast('دسته بعدی تست شد');
      } catch (e) { showToast(e.message, true); }
      finally { btn.disabled = false; btn.textContent = orig; }
    });

    // "شروع اسکن پیوسته": تا وقتی دکمه توقف زده نشود، هر چند ثانیه یک بار
    // دسته‌ی بعدی آی‌پی/پورت‌ها را تست می‌کند. چون Workerها نمی‌توانند در
    // پس‌زمینه بدون درخواست واقعی اجرا شوند، این حلقه از همین تب مرورگر ادمین
    // درخواست‌ها را پشت‌سرهم می‌فرستد — با بستن تب یا زدن «توقف»، متوقف می‌شود.
    const CLEAN_IP_CONTINUOUS_INTERVAL_MS = 4000;
    function setContinuousScanUi(active) {
      const btn = document.getElementById('cleanIpsContinuousBtn');
      const status = document.getElementById('cleanIpsContinuousStatus');
      if (active) {
        btn.textContent = '⏹️ توقف اسکن';
        status.textContent = 'در حال اسکن پیوسته… (تا زدن توقف ادامه دارد)';
      } else {
        btn.textContent = '▶️ شروع اسکن پیوسته';
        status.textContent = '';
      }
    }
    document.getElementById('cleanIpsContinuousBtn').addEventListener('click', async () => {
      if (cleanIpScanTimer) {
        clearInterval(cleanIpScanTimer);
        cleanIpScanTimer = null;
        setContinuousScanUi(false);
        return;
      }
      setContinuousScanUi(true);
      try { await runOneCleanIpBatch(); } catch (e) { showToast(e.message, true); }
      cleanIpScanTimer = setInterval(async () => {
        try { await runOneCleanIpBatch(); } catch (e) { /* یک خطای موقت نباید حلقه را متوقف کند */ }
      }, CLEAN_IP_CONTINUOUS_INTERVAL_MS);
    });
    ...
    loadOverview(); loadUsers(); loadBotUsers(); loadTrialSettings(); loadQuota(); loadPool(); loadSyncSecret(); loadCleanIps(); loadAutoCleanIpToggle(); loadCleanIpScanSettings(); loadDnsRules();
  })();
  </script>`
