...
export function dashboardPage(): Response {
  ...
  <div class="dash-shell">
    ...
    <div class="tab-panel" id="tab-pool">
      ...
      <div class="glass" style="margin-bottom:1.2rem;">
        <h3 style="margin-bottom:0.6rem;">🌐 آی‌پی‌های سالم (دستی)</h3>
        <p style="font-size:0.8rem; opacity:0.7; margin-bottom:0.8rem;">
          هیچ تست یا اسکن خودکاری انجام نمی‌شود. آی‌پی و پورتی که خودتان برای کاربران‌تان سالم/باز بودنش را
          تأیید کرده‌اید همین‌جا وارد کنید. وقتی گزینه‌ی زیر را فعال کنید، آدرس اتصال (add) کانفیگ‌ها به‌جای دامنه‌ی
          خود ورکر، یکی از همین آی‌پی:پورت‌ها (به‌صورت تصادفی از بین لیست) می‌شود؛ فیلدهای host/sni همچنان روی دامنه‌ی
          واقعی ورکر باقی می‌مانند، پس مسیریابی خراب نمی‌شود.
        </p>
        ...
        <p style="font-size:0.75rem; opacity:0.65; margin-top:0.6rem;">
  ...
  (function() {
    ...
    function renderCleanIps(ips) {
      ...
      tbody.innerHTML = ips.map(ip => {
        const flag = ip.country ? ((COUNTRY_FLAGS[ip.country] || '') + ' ' + ip.country) : '-';
