...
export function dashboardPage(): Response {
  ...
  (function() {
    ...
    // Shows any uncaught JS error or unhandled promise rejection directly on
    // the page as a big red banner, so it can be read/screenshotted on a
    // phone without needing desktop devtools. Safe to remove once the
    // underlying issue is found and fixed.
    function showDebugBanner(text) {
      ...
      if (!banner) {
        ...
        banner.style.cssText = 'position:fixed;top:0;left:0;right:0;z-index:99999;' +
          'background:#b00020;color:#fff;padding:12px;font:12px/1.5 monospace;' +
    ...
    window.addEventListener('error', function (e) {
      showDebugBanner('JS ERROR: ' + e.message + '\nfile: ' + e.filename + ':' + e.lineno + ':' + e.colno + '\n' + (e.error && e.error.stack ? e.error.stack : ''));
    });
    window.addEventListener('unhandledrejection', function (e) {
      showDebugBanner('UNHANDLED PROMISE REJECTION: ' + (e.reason && e.reason.message ? e.reason.message : String(e.reason)) + '\n' + (e.reason && e.reason.stack ? e.reason.stack : ''));
    });
    // ------------------------------------------------------------------------
    ...
    document.querySelectorAll('.tab-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        // --- TEMPORARY debug helper: report exactly what happened, on-page,
        // regardless of whether anything actually threw. Safe to remove once
        // the underlying issue is found and fixed.
        showDebugBanner('TAB CLICK: data-tab="' + btn.dataset.tab + '" → looked for #' + panelId +
          ' → found: ' + (!!panelEl) +
          (panelEl ? ' → classes before: [' + panelEl.className + ']' : ''));
        if (panelEl) {
          panelEl.classList.add('active');
          showDebugBanner('#' + panelId + ' classes after: [' + panelEl.className + '] → computed display: ' + (window.getComputedStyle ? window.getComputedStyle(panelEl).display : 'n/a'));
        }
        // ------------------------------------------------------------------
        closeSidebar();
        ...
        if (tab === 'overview') { Promise.resolve(loadOverview()).catch(() => {}); Promise.resolve(loadQuota()).catch(() => {}); Promise.resolve(loadOwnerSub()).catch(() => {}); }
    ...
    (function prefillCandidateInput() {
      const box = document.getElementById('candidateInput');
      if (box && !box.value.trim()) box.value = CURATED_CLEAN_IP_RANGES.join('\n');
    })();
    ...
    function reportLoadError(name, e) {
      ...
      banner.innerHTML = '⚠️ بارگذاری این بخش‌ها ناموفق بود — لطفاً صفحه را رفرش کنید یا اتصال (و VPN فعال روی گوشی) را بررسی کنید:<br>' + loadErrors.join('<br>');
    }
    [loadOverview, loadUsers, loadBotUsers, loadTrialSettings, loadQuota, loadPool, loadSyncSecret, loadOwnerSub, loadCleanIps, loadAutoCleanIpToggle, loadDnsRules]
      .forEach((fn) => {
        try {
          Promise.resolve(fn()).catch((e) => { reportLoadError(fn.name, e); showToast((fn.name) + ': ' + e.message, true); });
        } catch (e) { reportLoadError(fn.name, e); }
      });
