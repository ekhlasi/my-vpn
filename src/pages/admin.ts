...
export function dashboardPage(): Response {
    ...
    <div class="tab-panel" id="tab-vahidscan">
      <div class="glass vahid-scanner-card">
        ...
        <p class="help-text">
          ...
          رسمی کلادفلر انتخاب می‌شود، باز بودن پورت‌های تیک‌خورده‌ی زیر روی آن تست می‌شود و در صورت باز بودن،
          پینگ واقعی آن اندازه‌گیری می‌شود — تا وقتی روی «شروع اسکن» نزنید هیچ اسکنی انجام نمی‌شود. هر بار که
          دکمه فعال است چند آی‌پی به‌طور هم‌زمان تست می‌شوند و نتایج جمع می‌شوند؛ هر وقت خواستید با دکمه‌ی
          «توقف اسکن» متوقفش کنید. از میان نتایج، بهترین‌ها بر اساس پینگ (حداکثر ۱۵ مورد) پایین لیست می‌شوند —
          هر کدام را خواستید تیک بزنید و به لیست سرورهای نمایش‌داده‌شده به کاربران اضافه کنید.
        </p>
  ...
  (function() {
    ...
    // ---------- اسکنر وحید: admin-supplied reference scanner (unchanged
    // algorithm, see src/services/vahid-scanner.ts) ----------
    // Entirely start/stop-button driven — never runs on page load or on a
    // timer. Each click of "شروع اسکن" repeatedly calls one round of the
    // backend scanner (identical in substance to the reference script's own
    // /scanip endpoint: 5 parallel attempts per round) and accumulates
    // unique-by-IP hits until either 15 are collected or the admin presses
    // "توقف اسکن".
    ...
    document.getElementById('vahidScanBtn').addEventListener('click', async () => {
      ...
      const TARGET = 15;
      const MAX_ROUNDS = 30; // safety cap — each round is already a real network scan, not a fixed-cost call
      ...
      try {
        for (let round = 0; round < MAX_ROUNDS && collected.length < TARGET; round++) {
          statusText.textContent = 'در حال اسکن… دور ' + (round + 1) + ' — ' + collected.length + ' آی‌پی سالم تا اینجا پیدا شده.';
          const data = await api('/api/admin/vahid-scan/round', { method: 'POST', body: JSON.stringify({ ports: checkedPorts }), signal: vahidScanController.signal });
