...
export function dashboardPage(): Response {
  ...
  (function() {
    ...
    async function runDiscovery() {
      ...
      logBox.textContent = 'در حال اجرا… (نتیجه و لاگ کامل بعد از پایان کشف نمایش داده می‌شود)';
      try {
        const data = await api('/api/admin/discover-clean-ip', { method: 'POST', body: JSON.stringify({ country }) });
        ...
        if (data.results.length === 0) {
          ...
          statusText.textContent = 'مرحله ۲ (اعتبارسنجی مستقیم) با SNI = ' + data.sni + ' انجام شد — ' + healthy + ' از ' + data.results.length + ' کاندید سالم بود.';
        }
      } catch (e) {
        statusText.textContent = '';
        showToast(e.message, true);
