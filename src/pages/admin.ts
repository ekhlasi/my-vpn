...
export function dashboardPage(): Response {
  ...
  <div class="dash-shell">
    ...
    <div class="tab-panel" id="tab-pool">
      ...
      <div class="glass">
        ...
        <div style="margin-top:1.4rem; padding-top:1.2rem; border-top:1px solid var(--border);">
          <h3>🕵️ کشف آی‌پی سالم از طریق پروکسی</h3>
          <p class="help-text">
            به‌جای اسکن مستقیم لیست بالا، اینجا یکی از پروکسی‌های SOCKS4 هر کشور یک کوئری DNS/DoH به سمت دامنه‌ی
            همین داشبورد می‌زند تا آی‌پی edge کلادفلر مخصوص همان مسیر شبکه کشف شود (کاندید). سپس همان کاندیدها —
            <b>نه از طریق پروکسی، بلکه مستقیم از خود ورکر</b> — اعتبارسنجی می‌شوند تا مشخص شود واقعاً سالم هستند یا نه.
            جهت این مکانیزم عمدی است و نباید برعکس شود.
          </p>
          <div style="display:flex; gap:0.6rem; flex-wrap:wrap; align-items:center;">
            <select id="discoverCountrySelect" style="max-width:220px;">
            ...
            <button class="btn" id="discoverBtn" type="button">🕵️ شروع کشف + اعتبارسنجی</button>
          </div>
          <p class="help-text help-text-mt" id="discoverStatusText"></p>
          <div id="discoverLogBox" style="display:none; margin-top:0.6rem; max-height:220px; overflow-y:auto; background:#0b0f14; border:1px solid var(--border); border-radius:8px; padding:0.6rem 0.8rem; font-family:monospace; font-size:0.72rem; line-height:1.6; direction:ltr; text-align:left; white-space:pre-wrap;"></div>
        </div>
      </div>
    </div>
  ...
  (function() {
    ...
    // ---------- Discovery (proxy -> DoH -> candidate) + validation (direct) ----------
    async function runDiscovery() {
      const btn = document.getElementById('discoverBtn');
      const statusText = document.getElementById('discoverStatusText');
      const logBox = document.getElementById('discoverLogBox');
      const country = document.getElementById('discoverCountrySelect').value;
      btn.disabled = true;
      const orig = btn.textContent;
      btn.textContent = '⏳ در حال کشف از طریق پروکسی…';
      statusText.textContent = 'مرحله ۱: کشف کاندید از طریق پروکسی‌های ' + (country === 'ALL' ? 'همه کشورها' : country) + '…';
      ...
      try {
        const res = await fetch('/api/admin/discover-clean-ip', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ country }),
        });
        if (res.status === 401) { location.href = '/admin/login'; return; }
        if (!res.body) throw new Error('مرورگر شما از استریم پاسخ پشتیبانی نمی‌کند');
        ...
        if (!finalData) throw new Error('پاسخی از سرور دریافت نشد');
        if (!finalData.ok) throw new Error(finalData.error || 'خطا در کشف');

        renderScanResults(finalData.results);
        const healthy = finalData.results.filter((r) => r.success).length;
        if (finalData.results.length === 0) {
          statusText.textContent = 'هیچ کاندیدی از طریق پروکسی‌های ' + country + ' کشف نشد (پروکسی‌ها ممکن است در دسترس نباشند).';
        } else {
          statusText.textContent = 'مرحله ۲ (اعتبارسنجی مستقیم) با SNI = ' + finalData.sni + ' انجام شد — ' + healthy + ' از ' + finalData.results.length + ' کاندید سالم بود.';
        }
      } catch (e) {
        statusText.textContent = '';
        showToast(e.message, true);
