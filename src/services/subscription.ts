...
export function buildUserSubscription(
  ...
  opts: {
    ...
    /**
     * Per-slot enable/disable (requirement: "گزینه فعال یا غیرفعال هر کدوم"
     * — like a real VPN provider's panel where you can turn individual
     * servers on/off). Missing keys default to enabled (true), so old
     * callers that don't pass this at all keep behaving exactly as before.
     * A disabled pro slot (auto1/auto2/approved1-3) is simply omitted from
     * the returned list — the user's client just never sees that server
     * entry. A disabled trial slot does NOT return zero configs (a trial
     * user only has the one slot, and handing them nothing would look like
     * a broken/expired account); instead it falls back to a plain
     * non-clean-IP config, same as if no clean IP had been picked at all.
     */
    slotEnabled?: Partial<{
      auto1: boolean
      auto2: boolean
  ...
  const slotEnabled = opts.slotEnabled || {}
  /** Slot key for pro slot index i (0-4), matching the settings keys in db/queries.ts. */
  const PRO_SLOT_KEYS = ['auto1', 'auto2', 'approved1', 'approved2', 'approved3'] as const
  ...
  if (user.type === 'pro') {
    ...
    for (let i = 0; i < PRO_CONFIG_COUNT; i++) {
      // A disabled slot is skipped entirely — no entry is added for it, so
      // it never shows up in the user's client.
      if (slotEnabled[PRO_SLOT_KEYS[i]] === false) continue
    ...
    // auto one, then plain fallback. If the trial slot is disabled, skip
    // straight to the plain fallback (no clean-IP pick) rather than
    // omitting the config entirely.
    const trialOn = slotEnabled.trial !== false
    const pick = trialOn ? selectedCleanIps[0] || autoCleanIps[0] : undefined
    const flag = flagForColo(pick?.colo)
