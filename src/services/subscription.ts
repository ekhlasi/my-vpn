...
export interface SubscriptionOptions {
   ...
   */
  ipOverride?: string
  /** Port to dial when ipOverride is set (one of IRAN_VALID_PORTS in services/clean-ips.ts). Defaults to 443. */
  portOverride?: number
}
...
export function buildUserSubscription(
  ...
  opts: {
    ...
    poolHosts?: string[]
    /** Auto-discovered clean IP + port to dial instead of the worker's own domain (requirement #5). */
    cleanIp?: { ip: string; port: number } | null
  ...
  const ipOverride = opts.cleanIp?.ip || undefined
  const portOverride = opts.cleanIp?.port || undefined
