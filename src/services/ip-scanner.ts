...
/** Smallest CIDR prefix accepted per line (i.e. the biggest range allowed).
 *  /24 = 256 addresses — big enough to cover a typical published Cloudflare
 *  or ISP allocation block, small enough that expanding + scanning it stays
 *  well inside MAX_CANDIDATE_JOBS and a single Worker invocation's CPU/time
 *  budget. Anything larger (e.g. /16, /20) is rejected rather than silently
 *  truncated, so the admin knows to split it into smaller blocks themselves. */
...
/** Hard ceiling on how many ip×port combinations one "discover candidates" scan
 *  can run — this box always runs inside the Worker's own CPU/subrequest
 *  budget, so an admin pasting a huge list must be truncated rather than
 *  silently timing out or exhausting subrequests. */
export const MAX_CANDIDATE_JOBS = 400
 ...
 * a fresh, ad-hoc batch of ip (or small-CIDR-range) candidates the admin
 * pastes in right now — e.g. copied from a community "Germany clean IP"
 * list — and performs the exact same real TLS handshake against THIS
 * worker's own domain (via SNI) to see which ones actually reach it.
 * Nothing is fetched or guessed on the server side; every ip:port tested
 * comes from something the admin explicitly supplied (a single address or
 * a bounded /24-or-smaller range they typed in).
 */
export async function scanCandidates(
  candidates: CandidateInput[],
  sni: string,
  options: CandidateScanOptions,
): Promise<{ results: ScanResult[]; truncated: boolean }> {
  ...
  const truncated = jobs.length > MAX_CANDIDATE_JOBS
  const limited = truncated ? jobs.slice(0, MAX_CANDIDATE_JOBS) : jobs
  const results = await mapWithConcurrency(limited, SCAN_CONCURRENCY, async (j) => ({
    ...(await sniHandshake(j.ip, j.port, sni)),
    country: j.country ?? null,
  }))
  return { results: sortResults(results), truncated }
}
