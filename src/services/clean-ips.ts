...
import { countryForColo } from './cf-colos'
...
export interface CleanIpResult {
  ...
  error: string | null
  /** Cloudflare colo (data-center) code that answered this specific IP, e.g. "FRA". Null if the test failed before a response came back. */
  colo: string | null
  /** ISO 3166-1 alpha-2 country derived from `colo` (see services/cf-colos.ts), for grouping clean IPs by country in the admin panel. Null if the colo is unknown or missing. */
  country: string | null
}
...
/** Pulls the `colo=XXX` line out of a `/cdn-cgi/trace` response body (plain `key=value` lines, one per line). */
function parseColo(traceBody: string): string | null {
  const match = traceBody.match(/^colo=([A-Za-z0-9]+)$/m)
...
async function testOneIp(domain: string, ip: string): Promise<CleanIpResult> {
  ...
  try {
    ...
    if (!res.ok) return { ip, healthy: false, latencyMs, error: `HTTP ${res.status}`, colo: null, country: null }
    const colo = parseColo(await res.text())
    return { ip, healthy: true, latencyMs, error: null, colo, country: countryForColo(colo) }
  } catch (err: any) {
    return { ip, healthy: false, latencyMs: Date.now() - start, error: err?.message || 'network error', colo: null, country: null }
  }
}
